...
class UnetHook(nn.Module):
    ...
    def guidance_schedule_handler(self, x):
        # control_params 属性可能会被删除
        for param in getattr(self, 'control_params', []):
        ...
        def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
            ...
            if is_sdxl:
                ...
                if isinstance(revision_y1280, torch.Tensor):
                    ...
                    if any('ignore_prompt' in param.preprocessor['name'] for param in getattr(outer, 'control_params', [])) \
                            or (getattr(process, 'prompt', '') == '' and getattr(process, 'negative_prompt', '') == ''):
                        context = torch.zeros_like(context)

            # High-res fix
            # JACKSTRAW: outer.control_params 字段可能被restore删除故此调整
            for param in getattr(outer, 'control_params', []):
